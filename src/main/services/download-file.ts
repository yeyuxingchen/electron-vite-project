...
class Main {
  ...
  constructor(mainWindow: BrowserWindow, args: any) {
    const { downloadUrl, savePath } = args
    ...
    this.HistoryFilePath = savePath ? (savePath + '/file.example'): this.HistoryFilePath
...
export const downloadIpcHandlers = {
  ...
  handler: (event: Electron.IpcMainInvokeEvent, {downloadUrl, savePath}) => {
    new Main(BrowserWindow.fromWebContents(event.sender), {downloadUrl, savePath}).start()
  },
}
